...
fn p13699(){
  ...
  let mut arr:[u64;40]=[0;40];
  arr[0]=1;
  arr[1]=1;
  
  for i in 2..N+1{
    //t3= 02 11 20 = 5
    //t4= 03 12 21 30 = 10+ 2= 12
    //t5= 04 13 22 31 40 = 24+ 10+ 2 =36
    //t6= 05 14 23 32 41 50= 72+ 24+ 10= 106
    let mut result=0;
    for j in 0..i {
      result+= arr[j]*arr[i-j-1];
    }
    arr[i]=result;
  }
  println!("{}",arr[N]);
...
fn p7576(){
  ...
  let mut x:usize= N[1];
  let mut y:usize = N[0];
  let mut arr:[[i32;1010];1010] = [[0;1010];1010];
...
fn p1010(){
  ...
  let mut arr: [[u32;40];40]= [[0;40];40];

  for test in 0..N{

    read_vec!(vec as usize);
...
fn p1463() {
    read!(n as usize);
    let mut arr: [u32; 1000001] = [0; 1000001];
    arr[0] = 0;
    arr[1] = 0;
    arr[2] = 1;
    arr[3] = 1;
...
fn p2193(){
  //이친수
  //조건 1. 0으로 시작 x
  //조건 2. 1이 연속으로 나올 수 없음.
  read!(N as usize);
  let mut arr: [u64;95] = [0;95];
  let mut count: usize = 1;
  println!("{}", p2193fn(N,&mut arr));
}

fn p2193fn(N:usize, arr: &mut[u64])-> u64{
    if N==1{
      arr[N]=1;
    }else if N==2{
      arr[N]=1;
      ...
      arr[N]= p2193fn(N-2,arr)*2 + p2193fn(N-3,arr);
    }
...
impl Copy for p1463_struct {}

impl Clone for p1463_struct {
    fn clone(&self) -> p1463_struct {
        p1463_struct{count: self.count, val: self.val }
    }
}
impl p1463_struct{
  fn new () -> p1463_struct{
    p1463_struct {count: 0, val: 0 }
  }
}

fn p1463(){
  read!(N as u32);
  let mut arr = [p1463_struct::new();1000000];
  let mut num:usize=2;

  p1463fn(N, num,&mut arr);

}

fn p1463fn(N:u32, num:usize, arr:&mut [p1463_struct] )-> p1463_struct{
...
impl Copy for p9625_struct {}

impl Clone for p9625_struct {
    fn clone(&self) -> p9625_struct {
        p9625_struct{A: self.A, B: self.B }
    }
}
impl p9625_struct{
  fn new () -> p9625_struct{
    p9625_struct { A: 0, B: 0 }
  }
}

fn p9625() {
  read!(K as usize);
  let mut arr = [p9625_struct::new(); 46];

  let  result = p9625fn(K, &mut arr);
  println!("{} {}", result.A, result.B);
}

fn p9625fn(K: usize, arr:&mut [p9625_struct]) -> p9625_struct
  {
    if K==1{
      arr[K].A=0; arr[K].B=1;
    }else if K==2{
      ...
      arr[K].A = p9625fn(K-1, arr).B;
      arr[K].B = p9625fn(K-1, arr).A  + p9625fn(K-1, arr).B ;
    }
    return arr[K];
...
fn p9095() {
    ...
    for i in 0..t {
        read!(n as usize);
        println!("{}", p9095fn(n, &mut arr));
    }
}
fn p9095fn(n: usize, arr: &mut [u32]) -> u32 {
    if n == 0 {
        return 0;
    }
    if n == 1 {
    ...
    if arr[n] == 0 {
        arr[n] = p9095fn(n - 1, arr) + p9095fn(n - 2, arr) + p9095fn(n - 3, arr);
    }
    return arr[n];
}
*/
...
fn p9655() {
    read!(N as usize);
    let mut arr: [u32; 1001] = [0; 1001];

    if p9655fn(N, &mut arr) == 1 {
        println!("SK");
    } else {
        println!("CY");
    }
}
fn p9655fn(n: usize, arr: &mut [u32]) -> u32 {
    if n == 1 {
        return 1;
    }
    if n == 2 {
...
fn P2579() {
    read!(N as usize);
    let mut arr: [u32; 301] = [0; 301];
    let mut arr1: [u32; 301] = [0; 301];
    ...
    let mut count: u32 = 0;
    println!("{}", P2579fn(N, &mut arr, &mut arr1, &mut count));
}

fn P2579fn(n: usize, arr: &mut [u32], arr1: &mut [u32], count: &mut u32) -> u32 {
    let mut max: u32;
    if n == 1 {
        return arr[0];
    };
    ...
    if arr1[n] == 0 {
        if arr1[n - 1] >= arr1[n - 2] {
            arr1[n] = arr[n] + P2579fn(n - 1, arr, arr1, count);
        } else {
            arr1[n] = arr[n] + P2579fn(n - 2, arr, arr1, count);
        }
    }
    return arr1[n];
}
