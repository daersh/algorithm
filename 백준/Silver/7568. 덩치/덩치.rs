...
fn P7568(){
  ...
  let mut arr = vec![vec![0; 2]; 50];
  let mut i:usize;
  for i in 0..N {
    let mut insert= String::new();
    io::stdin().read_line(&mut insert).expect("err");
    let num:Vec<usize> = insert.trim().split_whitespace()
                        .map(|s| s.trim().parse().expect("err"))
                        .collect::<Vec<_>>();
    arr[i][0]= num[0] as u32;
    arr[i][1]= num[1] as u32;
  }
  let mut arr2=[0;50];
  let mut j:usize;
  let mut rank=1;
