...
fn P1978(){
    //첫줄에 N 개수 받음. 100 이하의 개수, 1000의하의 값들을 받음. 값들 중 소수의 개수 출력
    read!(n as usize);
    let mut count:u32 = 0;
    let mut insert= String::new();

    io::stdin().read_line(&mut insert).expect("er");
    let vec:Vec<u32> = insert.split_whitespace()
...
fn P1436(){// 666이 포함된 값을 저장하고 찾는 문제
    let mut vec: Vec<i64>= Vec::new();
    read!(insert as usize);
    for i in 666..100000000{
        if i.to_string().contains("666"){ // 문자열 666을 포함하는 값 찾기
            vec.push(i);
        }
        if vec.len()==insert{
            print!("{}",vec[insert-1]);
